...
import colour
from colour.models import matrix_RGB_to_RGB, RGB_COLOURSPACE_sRGB, RGB_COLOURSPACE_PROPHOTO_RGB
import numpy as np
import colour
...
#     assert is_valid_type(x) and is_valid_domain(x)
    m_srgb_to_prop = matrix_RGB_to_RGB(RGB_COLOURSPACE_sRGB, RGB_COLOURSPACE_PROPHOTO_RGB,
...
def prop_to_srgb_cat02(x):
    assert is_valid_type(x) and is_valid_domain(x)
    m_prop_to_srgb = matrix_RGB_to_RGB(RGB_COLOURSPACE_PROPHOTO_RGB, RGB_COLOURSPACE_sRGB,
                                       chromatic_adaptation_transform='CAT02')
...
def srgb_to_Lab_D50(img_srgb):
    ...
    XYZ = colour.models.RGB_to_XYZ(img_srgb.reshape(-1,3), 
                                         srgb_colourspace.whitepoint, 
                                         srgb_colourspace.whitepoint,
                                         srgb_colourspace.matrix_RGB_to_XYZ)
    Lab_srgb = colour.XYZ_to_Lab(XYZ, 
...
def lab_to_srgb_D50(Lab):
    ...
    img = colour.models.XYZ_to_RGB(XYZ, 
                 srgb_colourspace.whitepoint, 
                 srgb_colourspace.whitepoint,
...
def prop_to_Lab_D50(img_prop):
    ...
    XYZ = colour.models.RGB_to_XYZ(img_prop.reshape(-1,3), 
                                 prophotorgb_colourspace.whitepoint, 
                                 prophotorgb_colourspace.whitepoint,
                                 prophotorgb_colourspace.matrix_RGB_to_XYZ)
    Lab_prop = colour.XYZ_to_Lab(XYZ, 
                                 illuminant=ILLUMINANT_D50)
    # Scale: 
    ## L: 0-100
    ## a, b: -100 - 100
...
def prop_to_Lab_D65(img_prop):
    # img_srgb has dim = MxNx3
    # img_srgb scale: 0-1 and already decoded
    prophotorgb_colourspace = colour.models.PROPHOTO_RGB_COLOURSPACE.chromatically_adapt(ILLUMINANT_D65)
